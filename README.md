# KCase [![Build](https://github.com/jzlichking/KCase/actions/workflows/build.yml/badge.svg)](https://github.com/jzlichking/KCase/actions/workflows/build.yml)
...
镜像默认从 Docker Hub 拉取：`docker.io/jzlichking/kcase-{frontend,backend,mysql}`，同时也发布到 GitHub Container Registry：`ghcr.io/jzlichking/kcase-{frontend,backend,mysql}`。

```bash
mkdir kcase && cd kcase

# 1. 下载 compose 文件和 .env 模板
curl -O https://raw.githubusercontent.com/jzlichking/KCase/main/docker/docker-compose.yml
curl -o .env https://raw.githubusercontent.com/jzlichking/KCase/main/docker/.env.example

# 2. 编辑 .env：必改 MYSQL_ROOT_PASSWORD 和 MYSQL_PASSWORD（二者必须一致）
vim .env

# AI 生成能力可选；如需启用，填写 OpenAI 兼容接口配置，例如：
# AI_OPENAI_BASE_URL=https://api.openai.com/v1
# AI_OPENAI_API_KEY=sk-xxxx
# AI_OPENAI_MODEL_NAME=gpt-4o-mini
...
> 如需使用 GHCR，可在 `.env` 中把 `KCASE_REGISTRY` 改成 `ghcr.io/jzlichking`，再执行 `docker compose pull`。

### 方式二：源码本地构建（开发者）

```bash
# 1. 克隆并构建前端产物
git clone https://github.com/jzlichking/KCase.git
cd KCase
